...
namespace scls {
    ...
    Extendable_Int::Extendable_Int(unsigned long long i){a_parts[0] = i;i = i >> 32;a_parts.push_back(i);simplify();};
    Extendable_Int::Extendable_Int(const Extendable_Int& m){a_parts = m.a_parts;};
    ...
    bool Extendable_Int::is_bigger_than(Extendable_Int *c) {
        // Parts
    ...
    bool Extendable_Int::is_equal(Extendable_Int *c) {
        // Parts
        if(c->a_parts.size() != a_parts.size()){return false;}
    ...
    void Extendable_Int::divide(unsigned long long c){Extendable_Int i = Extendable_Int(c);divide(&i);}
    void Extendable_Int::divide(Extendable_Int* c) {
    ...
    void Extendable_Int::multiply(unsigned long long c){Extendable_Int i = Extendable_Int(c);multiply(&i);}
    ...
    std::string Extendable_Int::to_std_string() {
